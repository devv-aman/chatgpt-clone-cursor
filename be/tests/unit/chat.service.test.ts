...
describe('ChatService', () => {
  ...
  describe('createChatSession', () => {
    ...
    it('should create a chat session with custom title', async () => {
      ...
      const result = await chatService.createChatSession(MOCK_USER_ID, customTitle);
    ...
    it('should throw error if creation fails', async () => {
      ...
      await expect(
        chatService.createChatSession(MOCK_USER_ID)
      ).rejects.toThrow(ERROR_MESSAGES.CHAT.CREATE_FAILED);
    });
  });
  ...
  describe('getChatSession', () => {
    it('should return chat session if exists and owned by user', async () => {
      ...
      const result = await chatService.getChatSession(MOCK_CHAT_ID, MOCK_USER_ID);
  ...
  describe('saveMessage', () => {
    it('should save a message to the chat', async () => {
      const mockFrom = {
        insert: jest.fn().mockReturnThis(),
        select: jest.fn().mockReturnThis(),
        single: jest.fn().mockResolvedValue({ data: MOCK_MESSAGE, error: null }),
      };
    ...
    it('should save a message with model info', async () => {
      ...
      const mockFrom = {
        insert: jest.fn().mockReturnThis(),
        select: jest.fn().mockReturnThis(),
        single: jest.fn().mockResolvedValue({ data: messageWithModel, error: null }),
      };
  ...
  describe('getMessagesForChat', () => {
    it('should return messages for a chat', async () => {
      ...
      const mockMessagesFrom = {
        ...
        range: jest.fn().mockResolvedValue({ data: [MOCK_MESSAGE], error: null }),
      };
      ...
      const result = await chatService.getMessagesForChat(MOCK_CHAT_ID, MOCK_USER_ID);
  ...
  describe('stopChatStream', () => {
    it('should call streamService.stopStream', async () => {
      ...
      expect(streamService.stopStream).toHaveBeenCalledWith(MOCK_STREAM_ID, MOCK_USER_ID);
    });
  });
});
